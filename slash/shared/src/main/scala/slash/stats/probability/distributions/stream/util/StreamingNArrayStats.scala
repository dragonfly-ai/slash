...
package slash.stats.probability.distributions.stream.util
...
class StreamingNArrayStats(dimension:Int) {  // Why doesn't this extend Sampleable?
  ...
  def apply(c: NArray[Double], weight: Double = 1.0): StreamingNArrayStats = synchronized {
    s0 = s0 + weight
    for (i <- 0 until dimension) {
      val cv = c(i)
      val wci = cv * weight
  ...
  def average(): NArray[Double] = slash.vector.util.divided(s1, s0)

  private def componentVariance(s1d: Double, s2d: Double): Double = (s0 * s2d - s1d * s1d)/(s0 * (s0 - 1))

  def variance: NArray[Double] = NArray.tabulate[Double](dimension)(i => componentVariance(s1(i), s2(i)))

  def standardDeviation: NArray[Double] = NArray.tabulate[Double](dimension)(
    i => Math.sqrt(componentVariance(s1(i), s2(i)))
  )

  //override def toString: String = s"StreamingVectorStats(\n\ts0: $s0\n\ts1: ${NArray[Double](s1).render()}\n\ts2: ${NArray[Double](s2).render()}\n\tminValues: ${NArray[Double](minValues).render()}\n\tmaxValues: ${NArray[Double](maxValues).render()}\n\tAverage: ${average().render()}\n\tVariance: ${variance.render()}\n\tStandard Deviation: ${standardDeviation.render()})"
}
