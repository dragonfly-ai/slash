...
object WeightedVector extends Demonstrable {
  ...
  override def demo(implicit sb:StringBuilder = new StringBuilder()):StringBuilder = {
    val wv0 = WeightedVector(Vector(1.1, 2.5, 0.1), 0.5)
...
case class WeightedVector(unweighted: ai.dragonfly.math.vector.Vector, private var w: Double = 0.0) {
  def weight:Double = w
  def addWeight(w1: Double): WeightedVector = {
    w = w + w1
    this
  }
  def weighted: ai.dragonfly.math.vector.Vector = unweighted * weight
}