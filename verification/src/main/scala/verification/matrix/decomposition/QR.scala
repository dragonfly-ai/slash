...
object QR extends Verification {
  ...
  override def run: Unit = {
    ...
    println(s"\tComparing isFullRank: ${jqr.isFullRank} vs ${mqr.fullRank}")
    ...
    val solved = jqr.solve(solveJaMa)
    println(s"${solved.getRowDimension}x${solved.getColumnDimension}")
    println(s"\tComparing solve : ${Verification.matrixCompare(jqr.solve(solveJaMa), mqr.solve(solveMa))}")
...
//    println("\tWide Mat[11, 19] decomposition:")
...
//    println(s"\tComparing R : ${Verification.arrayCompare2D(jqrWide.getR.getArray, mqrWide.R.values)}")
    ...
    println(s"\tComparing isFullRank: ${jqrTall.isFullRank} vs ${mqrTall.fullRank}")
