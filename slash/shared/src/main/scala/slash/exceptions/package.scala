...
package object exceptions {
  ...
  case class VectorNormalizationException(s: String) extends Exception(s"Can't normalize $s")

  case class ExtraDimensionalAccessException(dim:Int, s:String, ci: Int) extends Exception({
    s"Index: $ci exceeds dimensionality of Euclidean object$dim: $s"
  })

  case class CannotExpressMatrixAsVector(rows:Int, columns:Int) extends Exception(
    s"To convert a Matrix to a Vector, one of its dimensions must be 1, but this matrix has dimensions: [${rows}x$columns]"
  )
